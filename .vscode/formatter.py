...
lunar_lander_rl_category: lldb.SBTypeCategory
...
def parray(debugger, command, result, internal_dict):
    """
    parray <variable>
    Print all elements of an ndarray::Array1<f32> in order.
    Works even if LLDB treats things as C++.
    """
    command = command.strip()
    if not command:
        print("Usage: parray <variable>")
        return

    frame = debugger.GetSelectedTarget().GetProcess().GetSelectedThread().GetSelectedFrame()
    print(f"Frame: {frame} captured")
    val = frame.FindVariable(command)
    print(f"Val: {val} captured")
    if not val.IsValid():
        print(f"Variable '{command}' not found")
        return

    try:
        ptr_val = val.GetChildMemberWithName("ptr").GetChildMemberWithName("pointer")
        len_val = val.GetChildMemberWithName("len")
        length = len_val.GetValueAsUnsigned()
        addr = ptr_val.GetValueAsUnsigned()
    except Exception as e:
        print(f"Failed to get pointer/len: {e}")
        return

    process = frame.GetThread().GetProcess()
    error = lldb.SBError()
    data = process.ReadMemory(addr, length * 4, error)  # f32 = 4 bytes
    if not error.Success():
        print(f"Memory read failed: {error.GetCString()}")
        return

    floats = struct.unpack(f"{length}f", data)
    print(f"{command} = [{', '.join(f'{x:.6g}' for x in floats)}]")

def initialize_category(debugger: lldb.SBDebugger):
    global module, lunar_lander_rl_category

    lunar_lander_rl_category = debugger.CreateCategory("BoltzmannSolver")
    lunar_lander_rl_category.SetEnabled(True)

    # attach_synthetic_to_type(
    #     ArrayBaseSynthProvider,
    #     r".*ArrayBase<.+>.*",  # match any module prefix and all nested generics
    #     True
    # )
    attach_synthetic_to_type(
        ArrayBaseSynthProvider,
    ...
    attach_summary_to_type(
        particle_summary_provider, "lunar_lander_rl::model::particle::Particle"
    )
...
def attach_synthetic_to_type(synth_class, type_name, is_regex=False):
    ...
    lunar_lander_rl_category.AddTypeSynthetic(
...
def attach_summary_to_type(summary_fn, type_name, is_regex=False):
    ...
    lunar_lander_rl_category.AddTypeSummary(
...
class ArrayBaseSynthProvider:
    def __init__(self, valobj: lldb.SBValue, dict={}):
        print("ArrayBaseSynthProvider __init__ called for", valobj.GetType().GetName())
        self.valobj = valobj
...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand('command script add -f formatter.parray parray')
    print("parray command registered. Usage: parray <Array1 variable>")
